...
## Verify
Canonical gate first: `npm run verify` (`node:test` unit tests on the pure helpers — 30 tests green as of the runner commit — then `bundle:bare` + `bundle:ui` build clean). Then on-device (Pixel owner + TCL joiner + iPhone), install-over-top (never uninstall):
