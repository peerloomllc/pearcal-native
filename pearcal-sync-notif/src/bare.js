...
function makeApply (groupId) {
  return async function apply (nodes, view, host) {
    ...
    for (const node of nodes) {
      ...
      // Detect whether this op came from a remote peer
      const nodeWriterKey = node.key ? b4a.toString(node.key, 'hex') : null
      ...
      if (val.op === 'put') {
        ...
        const isNewer = !existing || !existing.value.updatedAt || !val.value.updatedAt || val.value.updatedAt >= existing.value.updatedAt
        if (isNewer) {
