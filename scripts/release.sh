...
# ---------------------------------------------------------------------------
# 4b. Build desktop artifacts in parallel (Linux/Mac/Win)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4c. Build blind-seeder installers (auto-update assets)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4d. Build blind-seeder STORE artifacts (Docker image → ghcr + Start9 .s9pk)
# ---------------------------------------------------------------------------
...
try:
  ...
  gh release create "$RELEASE_TAG" "$APK_NAME" "${DESKTOP_ARTIFACTS[@]}" "${SEEDER_ARTIFACTS[@]}" \
    --title "$RELEASE_TAG" \
...
# ---------------------------------------------------------------------------
# 7c. Publish the Start9 community registry (points at the uploaded .s9pk)
# ---------------------------------------------------------------------------
  ...
  if command -v nak &>/dev/null; then
    ...
    if [ -n "$NOTES_SRC" ]; then
      ...
      while IFS= read -r _bline; do
        [ -z "$_bline" ] && continue
        _bline=$(printf '%s' "$_bline" | sed 's/^- \*\*//; s/\*\*[: ]*.*//; s/^- //')
        BULLETS="${BULLETS:+${BULLETS}$'\n'}• ${_bline}"
      done < <(grep '^- ' "$NOTES_SRC" | head -3)
    fi

    NOTE_CONTENT="${APP_NAME:-PearCal} ${RELEASE_TAG} is out!"$'\n\n'"${APP_TAGLINE:-}"
