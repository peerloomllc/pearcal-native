...
#                        (default seeder-launcher/start9/pearcal-seeder-v2.s9pk)
...
S9PK_V2="${S9PK_V2:-$START9_DIR/pearcal-seeder-v2.s9pk}"
...
if [ ! -f "$S9PK_V2" ] || [ -z "$_v2_cli" ]; then
  ...
  echo "  !! pearcal-seeder at all (0.3.5.x boxes are unaffected)." >&2
  if [ ! -f "$S9PK_V2" ]; then
  ...
  echo "==> upserting pearcal-seeder into the 0.4 registry payload for v$VERSION ..."
  ...
  if grep -qE '^//?package/v1/pearcal-seeder\.s9pk' "$RED"; then
    sed -i -E "s#(${RELEASE_REPO}/releases/download/)v[0-9.]+(/pearcal-seeder-v2\.s9pk)#\1v${VERSION}\2#g" "$RED"
    echo "    bumped existing /package/v1 _redirects rules to v$VERSION"
  else
    {
      echo "/package/v1/pearcal-seeder.s9pk  ${ASSET_BASE}/v${VERSION}/pearcal-seeder-v2.s9pk  302"
      echo "//package/v1/pearcal-seeder.s9pk  ${ASSET_BASE}/v${VERSION}/pearcal-seeder-v2.s9pk  302"
    } >> "$RED"
    echo "    appended /package/v1 _redirects rules for pearcal-seeder v$VERSION"
  fi
  ...
  ( cd "$WEBSITE_DIR"
    ...
    if [ -z "$(gh pr list --head "$BRANCH" --state open --json number --jq '.[0].number' 2>/dev/null)" ]; then
      gh pr create --base "$base" --head "$BRANCH" \
        ...
        --body "Upserted the \`/package/v0\` (StartOS 0.3.5.x) metadata and the \`/registry/0.4/index.json\` (StartOS 0.4+) payload, plus \`_redirects\`, for pearcal-seeder v${VERSION} — merged alongside the other listed packages. Auto-generated by publish-start9-registry.sh; merging deploys the registry (Cloudflare)." \
        >/dev/null
    fi
